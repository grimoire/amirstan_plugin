...
std::vector<PluginField> TorchFlipPluginDynamicCreator::mPluginAttributes(
    {PluginField("mode"), PluginField("dims")});

TorchFlipPluginDynamic::TorchFlipPluginDynamic(const std::string &name,
                                               const nvinfer1::Dims &dims)
    : mLayerName(name), mDims(dims) {}
...
nvinfer1::IPluginV2DynamicExt *TorchFlipPluginDynamic::clone() const {
  TorchFlipPluginDynamic *plugin =
      new TorchFlipPluginDynamic(mLayerName, mDims);
...
bool TorchFlipPluginDynamic::supportsFormatCombination(
...
int TorchFlipPluginDynamic::enqueue(
  ...
  switch (data_type) {
    case nvinfer1::DataType::kFLOAT:
      torch_flip<float>((float *)outputs[0], (float *)inputs[0],
                        &(input_dims.d[0]), input_dims.nbDims, &(mDims.d[0]),
                        mDims.nbDims, stream);
      break;
...
nvinfer1::DataType TorchFlipPluginDynamic::getOutputDataType(
...
int TorchFlipPluginDynamic::getNbOutputs() const { return 1; }

int TorchFlipPluginDynamic::initialize() { return 0; }
...
IPluginV2 *TorchFlipPluginDynamicCreator::createPlugin(
  ...
  TorchFlipPluginDynamic *plugin = new TorchFlipPluginDynamic(name, dims);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}

IPluginV2 *TorchFlipPluginDynamicCreator::deserializePlugin(
...
void TorchFlipPluginDynamicCreator::setPluginNamespace(
