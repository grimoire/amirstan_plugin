...
nvinfer1::IPluginV2DynamicExt* RoiPoolPluginDynamic::clone() const PLUGIN_NOEXCEPT
{
    RoiPoolPluginDynamic* plugin =
        new RoiPoolPluginDynamic(mLayerName, mOutSize, mFeatmapStrides, mRoiScaleFactor, mFinestScale);
    plugin->setPluginNamespace(getPluginNamespace());

    return plugin;
}

nvinfer1::DimsExprs RoiPoolPluginDynamic::getOutputDimensions(int                        outputIndex,
...
bool RoiPoolPluginDynamic::supportsFormatCombination(int                               pos,
...
void RoiPoolPluginDynamic::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* inputs,
                                           int                                      nbInputs,
...
size_t RoiPoolPluginDynamic::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                              int                               nbInputs,
...
nvinfer1::DataType RoiPoolPluginDynamic::getOutputDataType(int                       index,
...
int RoiPoolPluginDynamic::getNbOutputs() const PLUGIN_NOEXCEPT
{
    return 1;
}
...
{
    return mFeatmapStrides.size() * sizeof(float) + sizeof(mOutSize) + sizeof(mRoiScaleFactor) + sizeof(mFinestScale)
           + sizeof(int);
}
...
const char* RoiPoolPluginDynamicCreator::getPluginVersion() const PLUGIN_NOEXCEPT
{
    return PLUGIN_VERSION;
}

IPluginV2* RoiPoolPluginDynamicCreator::createPlugin(const char* name, const PluginFieldCollection* fc) PLUGIN_NOEXCEPT
{
    int                outSize = 7;
    ...
    RoiPoolPluginDynamic* plugin = new RoiPoolPluginDynamic(name, outSize, featmapStrides, roiScaleFactor, finestScale);
    plugin->setPluginNamespace(getPluginNamespace());
    return plugin;
}
