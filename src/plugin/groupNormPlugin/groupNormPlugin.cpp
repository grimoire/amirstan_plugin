...
SET_PLUGIN_NAME_VERSION(GroupNormPluginDynamic, "1")

GroupNormPluginDynamic::GroupNormPluginDynamic(const std::string& name, int num_groups, float eps):
...
nvinfer1::IPluginV2DynamicExt* GroupNormPluginDynamic::clone() const PLUGIN_NOEXCEPT
{
    GroupNormPluginDynamic* plugin = new GroupNormPluginDynamic(mLayerName, mNumGroups, mEps);
    plugin->setPluginNamespace(getPluginNamespace());

    return plugin;
}

nvinfer1::DimsExprs GroupNormPluginDynamic::getOutputDimensions(int                        outputIndex,
...
bool GroupNormPluginDynamic::supportsFormatCombination(int                               pos,
...
void GroupNormPluginDynamic::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* inputs,
                                             int                                      nbInputs,
...
size_t GroupNormPluginDynamic::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                                int                               nbInputs,
                                    ...
                                    cudaStream_t                      stream) PLUGIN_NOEXCEPT
{
    ...
    switch (data_type) {
        case nvinfer1::DataType::kFLOAT:
            compute_group_norm<float>((float*)outputs[0],
                                      (float*)inputs[0],
                                      batch_size,
                                      mNumGroups,
                                      inputChannel,
...
nvinfer1::DataType GroupNormPluginDynamic::getOutputDataType(int                       index,
...
int GroupNormPluginDynamic::getNbOutputs() const PLUGIN_NOEXCEPT
{
    return 1;
}
...
IPluginV2* GroupNormPluginDynamicCreator::createPlugin(const char*                  name,
                                                       const PluginFieldCollection* fc) PLUGIN_NOEXCEPT
{
    ...
    GroupNormPluginDynamic* plugin = new GroupNormPluginDynamic(name, num_groups, eps);
    plugin->setPluginNamespace(getPluginNamespace());
    return plugin;
}
}  // namespace plugin
