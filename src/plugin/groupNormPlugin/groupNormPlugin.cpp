...
std::vector<PluginField> GroupNormPluginDynamicCreator::mPluginAttributes(
    {PluginField("num_groups"), PluginField("num_channels"), PluginField("W"),
     PluginField("B"), PluginField("eps")});

GroupNormPluginDynamic::GroupNormPluginDynamic(const std::string &name,
                                               int num_groups, int num_channels,
                                               float eps,
...
nvinfer1::IPluginV2DynamicExt *GroupNormPluginDynamic::clone() const {
  GroupNormPluginDynamic *plugin = new GroupNormPluginDynamic(
      mLayerName, mNumGroups, mNumChannels, mEps, mW, mB);
  ...
  switch (data_type) {
    case nvinfer1::DataType::kFLOAT:
      compute_group_norm<float>((float *)outputs[0], (float *)inputs[0],
                                batch_size, mNumGroups, inputChannel,
                                inputWidth * inputHeight, mEps, (float *)mWdev,
                                (float *)mBdev, stream, workSpace);
      break;
...
size_t GroupNormPluginDynamic::getSerializationSize() const {
  ...
  return wordSize * mNumParamsW + wordSize * mNumParamsB +
         sizeof(mNumChannels) + sizeof(mNumGroups) + sizeof(mNumParamsW) +
         sizeof(mNumParamsB) + sizeof(mEps);
}
...
IPluginV2 *GroupNormPluginDynamicCreator::createPlugin(
  ...
  GroupNormPluginDynamic *plugin =
      new GroupNormPluginDynamic(name, num_groups, num_channels, eps, W, B);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}
