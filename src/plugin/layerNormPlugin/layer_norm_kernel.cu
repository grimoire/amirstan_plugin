...
__global__ void layer_norm_kernel(T *output, const T *input, size_t input_size,
                                  int norm_size, int layer_size, T eps, T *mean,
                                  T *var, const T *weight, const T *bias) {
...
template <typename T>
void compute_layer_norm(T *output, const T *input, int norm_size,
                        int layer_size, T eps, const T *weight, const T *bias,
  ...
  layer_norm_kernel<T><<<GET_BLOCKS(input_size), CUDA_NUM_THREADS, 0, stream>>>(
      output, input, input_size, norm_size, layer_size, eps, mean, var, weight,
      bias);
}
