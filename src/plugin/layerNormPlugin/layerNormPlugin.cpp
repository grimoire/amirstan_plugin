...
std::vector<PluginField> LayerNormPluginDynamicCreator::mPluginAttributes(
    {PluginField("normalized_shape"), PluginField("W"), PluginField("B"),
     PluginField("eps")});

LayerNormPluginDynamic::LayerNormPluginDynamic(
    const std::string &name, const nvinfer1::Dims &normalized_shape, float eps,
...
nvinfer1::IPluginV2DynamicExt *LayerNormPluginDynamic::clone() const {
  LayerNormPluginDynamic *plugin =
      new LayerNormPluginDynamic(mLayerName, mNormalizedShape, mEps, mW, mB);
...
int LayerNormPluginDynamic::enqueue(
  ...
  compute_layer_norm<float>((float *)outputs[0], (float *)inputs[0], norm_size,
                            layer_size, mEps, (float *)mWdev, (float *)mBdev,
                            stream, workSpace);
...
int LayerNormPluginDynamic::getNbOutputs() const { return 1; }
...
size_t LayerNormPluginDynamic::getSerializationSize() const {
  ...
  return wordSize * mNumParamsW + wordSize * mNumParamsB +
         sizeof(mNormalizedShape) + sizeof(mNumParamsW) + sizeof(mNumParamsB) +
         sizeof(mEps);
}
...
IPluginV2 *LayerNormPluginDynamicCreator::createPlugin(
  ...
  LayerNormPluginDynamic *plugin =
      new LayerNormPluginDynamic(name, normalizedShape, eps, W, B);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}

IPluginV2 *LayerNormPluginDynamicCreator::deserializePlugin(
...
void LayerNormPluginDynamicCreator::setPluginNamespace(
