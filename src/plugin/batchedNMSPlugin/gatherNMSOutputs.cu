...
pluginStatus_t gatherNMSOutputs_gpu(
    ...
    const void *bboxData, void *numDetections, void *nmsedBoxes,
    void *nmsedScores, void *nmsedClasses, bool clipBoxes) {
  ...
  gatherNMSOutputs_kernel<T_BBOX, T_SCORE, BS><<<GS, BS, 0, stream>>>(
      ...
      (int *)numDetections, (T_BBOX *)nmsedBoxes, (T_BBOX *)nmsedScores,
      (T_BBOX *)nmsedClasses, clipBoxes);
...
pluginStatus_t gatherNMSOutputs(cudaStream_t stream, const bool shareLocation,
                                ...
                                void *numDetections, void *nmsedBoxes,
                                void *nmsedScores, void *nmsedClasses,
                                bool clipBoxes) {
  ...
  for (unsigned i = 0; i < nmsOutFuncVec.size(); ++i) {
    if (lc == nmsOutFuncVec[i]) {
      ...
      return nmsOutFuncVec[i].function(
          stream, shareLocation, numImages, numPredsPerClass, numClasses, topK,
          keepTopK, indices, scores, bboxData, numDetections, nmsedBoxes,
          nmsedScores, nmsedClasses, clipBoxes);
    }
  }
