...
pluginStatus_t detectionInference(
    cudaStream_t stream, int N, int C1, int C2, bool shareLocation,
    bool varianceEncodedInTarget, int backgroundLabelId, int numPredsPerClass,
    int numClasses, int topK, int keepTopK, float confidenceThreshold,
    float nmsThreshold, CodeTypeSSD codeType, DataType DT_BBOX,
    const void* locData, const void* priorData, DataType DT_SCORE,
    const void* confData, void* keepCount, void* topDetections, void* workspace,
    bool isNormalized = true, bool confSigmoid = false

);

pluginStatus_t gatherTopDetections(cudaStream_t stream, bool shareLocation,
...
pluginStatus_t decodeBBoxes(cudaStream_t stream, int nthreads,
                            CodeTypeSSD code_type,
                            bool variance_encoded_in_target, int num_priors,
                            bool share_location, int num_loc_classes,
                            int background_label_id, bool clip_bbox,
                            DataType DT_BBOX, const void* loc_data,
                            const void* prior_data, void* bbox_data);

size_t normalizePluginWorkspaceSize(bool acrossSpatial, int C, int H, int W);

pluginStatus_t normalizeInference(cudaStream_t stream, cublasHandle_t handle,
                                  bool acrossSpatial, bool channelShared, int N,
                                  int C, int H, int W, float eps,
                                  const void* scale, const void* inputData,
                                  void* outputData, void* workspace);

pluginStatus_t priorBoxInference(cudaStream_t stream, PriorBoxParameters param,
                                 int H, int W, int numPriors,
                                 int numAspectRatios, const void* minSize,
                                 const void* maxSize, const void* aspectRatios,
                                 void* outputData);

pluginStatus_t reorgInference(cudaStream_t stream, int batch, int C, int H,
                              int W, int stride, const void* input,
                              void* output);

pluginStatus_t anchorGridInference(cudaStream_t stream,
                                   GridAnchorParameters param,
                                   int numAspectRatios,
                                   const void* aspectRatios, const void* scales,
                                   void* outputData);

pluginStatus_t regionInference(cudaStream_t stream, int batch, int C, int H,
                               int W, int num, int coords, int classes,
                               bool hasSoftmaxTree,
...
pluginStatus_t generateAnchors(
    cudaStream_t stream,
...
// BBD2P
pluginStatus_t bboxDeltas2Proposals(
    cudaStream_t stream,
    int N,                 // batch size
    ...
    void* scores);  // output scores (the score associated with too small box
                    // will be set to -inf)
...
// WORKSPACE SIZES
size_t proposalsForwardNMSWorkspaceSize(int N, int A, int H, int W,
                                        int nmsMaxOut);
...
size_t proposalsInferenceWorkspaceSize(int N, int A, int H, int W,
                                       int nmsMaxOut);

size_t RPROIInferenceFusedWorkspaceSize(int N, int A, int H, int W,
                                        int nmsMaxOut);

// PROPOSALS INFERENCE
pluginStatus_t proposalsInference(
    cudaStream_t stream, int N, int A, int H, int W, int featureStride,
    int preNmsTop, int nmsMaxOut, float iouThreshold, float minBoxSize,
    const float* imInfo, const float* anchors, DataType tScores,
    DLayout_t lScores, const void* scores, DataType tDeltas, DLayout_t lDeltas,
    const void* deltas, void* workspace, DataType tRois, void* rois);

// EXTRACT FG SCORES
pluginStatus_t extractFgScores(cudaStream_t stream, int N, int A, int H, int W,
                               DataType tScores, DLayout_t lScores,
                               const void* scores, DataType tFgScores,
                               DLayout_t lFgScores, void* fgScores);

// ROI INFERENCE
pluginStatus_t roiInference(
    cudaStream_t stream,
    ...
    const float spatialScale, const DataType tRois, const void* rois,
    const DataType tFeatureMap, const DLayout_t lFeatureMap,
    const void* featureMap, const DataType tTop, const DLayout_t lTop,
    void* top);
...
pluginStatus_t roiForward(cudaStream_t stream,
                          ...
                          float spatialScale, DataType tRois, const void* rois,
                          DataType tFeatureMap, DLayout_t lFeatureMap,
                          const void* featureMap, DataType tTop, DLayout_t lTop,
                          void* top, int* maxIds);

// RP ROI Fused INFERENCE
pluginStatus_t RPROIInferenceFused(
    cudaStream_t stream, int N, int A, int C, int H, int W, int poolingH,
    int poolingW, int featureStride, int preNmsTop, int nmsMaxOut,
    float iouThreshold, float minBoxSize, float spatialScale,
    const float* imInfo, const float* anchors, DataType tScores,
    DLayout_t lScores, const void* scores, DataType tDeltas, DLayout_t lDeltas,
    const void* deltas, DataType tFeatureMap, DLayout_t lFeatureMap,
    const void* featureMap, void* workspace, DataType tRois, void* rois,
    DataType tTop, DLayout_t lTop, void* top);

// GENERATE ANCHORS CPU
pluginStatus_t generateAnchors_cpu(int numRatios, float* ratios, int numScales,
                                   float* scales, int baseSize, float* anchors);

int cropAndResizeInference(cudaStream_t stream, int n, const void* image,
                           const void* rois, int batch_size, int input_height,
                           int input_width, int num_boxes, int crop_height,
                           int crop_width, int depth, void* output);

int proposalInference_gpu(cudaStream_t stream, const void* rpn_prob,
                          const void* rpn_regr, int batch_size,
                          int input_height, int input_width, int rpn_height,
                          int rpn_width, int MAX_BOX_NUM, int RPN_PRE_NMS_TOP_N,
                          float* ANCHOR_SIZES, int anc_size_num,
                          float* ANCHOR_RATIOS, int anc_ratio_num,
                          float rpn_std_scaling, int rpn_stride,
                          float bbox_min_size, float nms_iou_threshold,
                          void* workspace, void* output);

size_t _get_workspace_size(int N, int anc_size_num, int anc_ratio_num, int H,
                           int W, int nmsMaxOut);
