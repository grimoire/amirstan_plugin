...
Delta2BBoxPluginDynamic::Delta2BBoxPluginDynamic(const std::string&        name,
    ...
    PluginDynamicBase(name), mMinNumBBox(minNumBBox), mTargetMeans(targetMeans), mTargetStds(targetStds)
{
}
...
nvinfer1::IPluginV2DynamicExt* Delta2BBoxPluginDynamic::clone() const PLUGIN_NOEXCEPT
{
    Delta2BBoxPluginDynamic* plugin = new Delta2BBoxPluginDynamic(mLayerName, mMinNumBBox, mTargetMeans, mTargetStds);
    plugin->setPluginNamespace(getPluginNamespace());

    return plugin;
}

nvinfer1::DimsExprs Delta2BBoxPluginDynamic::getOutputDimensions(int                        outputIndex,
                                                                 ...
                                                                 nvinfer1::IExprBuilder&    exprBuilder) PLUGIN_NOEXCEPT
{
    ...
    auto final_bbox =
        exprBuilder.operation(nvinfer1::DimensionOperation::kMAX, *exprBuilder.constant(mMinNumBBox), *out_bbox);
...
bool Delta2BBoxPluginDynamic::supportsFormatCombination(int                               pos,
                                                        ...
                                                        int                               nbOutputs) PLUGIN_NOEXCEPT
{
    ...
    if (nbInputs == 4 && pos == 3) {
        return in[3].type == nvinfer1::DataType::kINT32 && in[3].format == nvinfer1::TensorFormat::kLINEAR;
    }

    return false;
}

void Delta2BBoxPluginDynamic::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* inputs,
                                              int                                      nbInputs,
...
size_t Delta2BBoxPluginDynamic::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                                 int                               nbInputs,
...
int Delta2BBoxPluginDynamic::enqueue(const nvinfer1::PluginTensorDesc* inputDesc,
                                     ...
                                     cudaStream_t                      stream) PLUGIN_NOEXCEPT
{
    ...
    if (num_inboxes != num_outboxes) {
        fill_zeros<float, float>((float*)out_cls,
                                 (float*)out_bbox,
                                 outputDesc[0].dims.d[0] * outputDesc[0].dims.d[1] * outputDesc[0].dims.d[2],
                                 outputDesc[1].dims.d[0] * outputDesc[1].dims.d[1] * outputDesc[1].dims.d[2],
                                 stream);
    }
    cudaMemcpyAsync(
        out_cls, in_cls, batch_size * num_inboxes * num_classes * sizeof(float), cudaMemcpyDeviceToDevice, stream);
    delta2bbox<float>((float*)out_bbox,
                      (float*)in_bbox,
                      (float*)anchor,
                      clip_range,
                      batch_size,
...
nvinfer1::DataType Delta2BBoxPluginDynamic::getOutputDataType(int                       index,
...
int Delta2BBoxPluginDynamic::getNbOutputs() const PLUGIN_NOEXCEPT
{
    return 2;
}
...
{
    return mTargetMeans.size() * sizeof(float) + mTargetStds.size() * sizeof(float) + sizeof(mMinNumBBox);
}
...
Delta2BBoxPluginDynamicCreator::Delta2BBoxPluginDynamicCreator()
{
    mPluginAttributes = std::vector<PluginField>(
        {PluginField("min_num_bbox"), PluginField("target_means"), PluginField("target_stds")});

    mFC.nbFields = mPluginAttributes.size();
    mFC.fields   = mPluginAttributes.data();
}

const char* Delta2BBoxPluginDynamicCreator::getPluginName() const PLUGIN_NOEXCEPT
{
    return PLUGIN_NAME;
}

const char* Delta2BBoxPluginDynamicCreator::getPluginVersion() const PLUGIN_NOEXCEPT
{
    return PLUGIN_VERSION;
}

IPluginV2* Delta2BBoxPluginDynamicCreator::createPlugin(const char*                  name,
                                                        const PluginFieldCollection* fc) PLUGIN_NOEXCEPT
{
    ...
    Delta2BBoxPluginDynamic* plugin = new Delta2BBoxPluginDynamic(name, minNumBBox, targetMeans, targetStds);
    plugin->setPluginNamespace(getPluginNamespace());
    return plugin;
}
