...
inline bool next_symbol(const std::string& exp, int start_pos, int& next_pos,
                        int& symbol) {
  // end of string
...
const nvinfer1::IDimensionExpr* get_value_impl(
    const std::string& exp, const nvinfer1::DimsExprs* inputs, int start_pos,
    int& next_pos, nvinfer1::IExprBuilder& exprBuilder);
const nvinfer1::IDimensionExpr* parse_expression_impl(
    const std::string& exp, const nvinfer1::DimsExprs* inputs, int start_pos,
    int& next_pos, nvinfer1::IExprBuilder& exprBuilder);
...
const nvinfer1::IDimensionExpr* get_value_impl(
  ...
  if (isnumber) {
    ...
    if (isalpha(symbol)) {
      ...
      result = parse_expression_impl(exp, inputs, start_pos + 1, next_pos,
                                     exprBuilder);
...
const nvinfer1::IDimensionExpr* parse_expression_impl(
  ...
  // get init value
  auto return_value =
      get_value_impl(exp, inputs, next_pos, next_pos, exprBuilder);
  ...
  for (int i = next_pos; i < exp.size(); ++i) {
    ...
    switch (chr_sym) {
      ...
      case '-':
        ...
        break;
      case '*':
        return_value = exprBuilder.operation(
            nvinfer1::DimensionOperation::kPROD, *return_value, *result);
        break;
      case '/':
        return_value = exprBuilder.operation(
            nvinfer1::DimensionOperation::kFLOOR_DIV, *return_value, *result);
        break;
      default:
        break;
    }
