...
std::vector<PluginField> ExViewPluginDynamicCreator::mPluginAttributes(
    {PluginField("dim_expression")});

ExViewPluginDynamic::ExViewPluginDynamic(const std::string &name,
                                         std::vector<std::string> dimExpression)
    : mLayerName(name), mDimExpression(dimExpression) {}
...
nvinfer1::IPluginV2DynamicExt *ExViewPluginDynamic::clone() const {
  ExViewPluginDynamic *plugin =
      new ExViewPluginDynamic(mLayerName, mDimExpression);
...
nvinfer1::DimsExprs ExViewPluginDynamic::getOutputDimensions(
  ...
  for (int i = 0; i < mDimExpression.size(); ++i) {
    outputDims.d[i] =
        parse_expression(mDimExpression[i], inputs + 1, exprBuilder);
  }

  return outputDims;
}

bool ExViewPluginDynamic::supportsFormatCombination(
...
int ExViewPluginDynamic::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  cudaMemcpyAsync(outputs[0], inputs[0], copy_size * wordSize,
                  cudaMemcpyDeviceToDevice, stream);

  return 0;
}

nvinfer1::DataType ExViewPluginDynamic::getOutputDataType(
    int index, const nvinfer1::DataType *inputTypes, int nbInputs) const {
  return inputTypes[0];
}

// IPluginV2 Methods
const char *ExViewPluginDynamic::getPluginType() const { return PLUGIN_NAME; }

const char *ExViewPluginDynamic::getPluginVersion() const {
  return PLUGIN_VERSION;
}

int ExViewPluginDynamic::getNbOutputs() const { return 1; }

int ExViewPluginDynamic::initialize() { return 0; }
...
size_t ExViewPluginDynamic::getSerializationSize() const {
  ...
  return sizeof(int) + str_length * sizeof(char);
}
...
IPluginV2 *ExViewPluginDynamicCreator::createPlugin(
  ...
  for (int i = 0; i < fc->nbFields; i++) {
    ...
    if (field_name.compare("dim_expression") == 0) {
      const char *chr_dim_exp_data =
          static_cast<const char *>(fc->fields[i].data);
  ...
  ExViewPluginDynamic *plugin = new ExViewPluginDynamic(name, dim_exp);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}

IPluginV2 *ExViewPluginDynamicCreator::deserializePlugin(const char *name,
