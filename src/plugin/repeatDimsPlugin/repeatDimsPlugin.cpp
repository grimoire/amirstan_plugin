...
std::vector<PluginField> RepeatDimsPluginDynamicCreator::mPluginAttributes(
    {PluginField("repeat_dims")});

RepeatDimsPluginDynamic::RepeatDimsPluginDynamic(
    const std::string &name, const nvinfer1::Dims &repeatDims)
    : mLayerName(name), mRepeatDims(repeatDims) {}

RepeatDimsPluginDynamic::RepeatDimsPluginDynamic(const std::string name,
                                                 const void *data,
                                                 size_t length)
    : mLayerName(name) {
...
nvinfer1::IPluginV2DynamicExt *RepeatDimsPluginDynamic::clone() const {
  RepeatDimsPluginDynamic *plugin =
      new RepeatDimsPluginDynamic(mLayerName, mRepeatDims);
...
nvinfer1::DimsExprs RepeatDimsPluginDynamic::getOutputDimensions(
  ...
  if (nbInputs == 1) {
    ...
    for (int i = 0; i < inputs[0].nbDims; ++i) {
      outputDims.d[i + dim_offset] = exprBuilder.operation(
          nvinfer1::DimensionOperation::kPROD, *(inputs[0].d[i]),
...
bool RepeatDimsPluginDynamic::supportsFormatCombination(
    ...
    case 1:
      if (nbInputs == 1) {
        ...
        return (in[1].type == nvinfer1::DataType::kFLOAT &&
               ...
               (in[1].type == nvinfer1::DataType::kHALF &&
                in[1].format == nvinfer1::TensorFormat::kLINEAR) ||
               (in[1].type == nvinfer1::DataType::kINT32 &&
                in[1].format == nvinfer1::TensorFormat::kLINEAR);
      }
    case 2:
      return out[0].type == in[0].type && out[0].format == in[0].format;
...
int RepeatDimsPluginDynamic::enqueue(
...
nvinfer1::DataType RepeatDimsPluginDynamic::getOutputDataType(
...
int RepeatDimsPluginDynamic::getNbOutputs() const { return 1; }

int RepeatDimsPluginDynamic::initialize() { return 0; }
...
IPluginV2 *RepeatDimsPluginDynamicCreator::createPlugin(
  ...
  RepeatDimsPluginDynamic *plugin =
      new RepeatDimsPluginDynamic(name, repeat_dims);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}

IPluginV2 *RepeatDimsPluginDynamicCreator::deserializePlugin(
...
void RepeatDimsPluginDynamicCreator::setPluginNamespace(
