...
nvinfer1::IPluginV2DynamicExt* DeformableConvPluginDynamic::clone() const PLUGIN_NOEXCEPT
{
    DeformableConvPluginDynamic* plugin =
        new DeformableConvPluginDynamic(mLayerName, mStride, mPadding, mDilation, mDeformableGroup, mGroup);
...
nvinfer1::DimsExprs
DeformableConvPluginDynamic::getOutputDimensions(int                        outputIndex,
...
bool DeformableConvPluginDynamic::supportsFormatCombination(int                               pos,
                                                            const nvinfer1::PluginTensorDesc* inOut,
                                                            int                               nbInputs,
                                                            int                               nbOutputs) PLUGIN_NOEXCEPT
{
    if (pos == 0) {
        return inOut[0].type == DataType::kFLOAT && inOut[0].format == nvinfer1::TensorFormat::kLINEAR;
    }
    else {
                                                     ...
                                                     int                               nbOutputs) const PLUGIN_NOEXCEPT
{
    ...
    size_t col_size = amirstan::common::getAlignedSize(nInputPlane * kW * kH * im2col_step * outputHeight * outputWidth
                                                       * sizeof_dtype);

    size_t out_size = 0;
    if (im2col_step != 1)
        out_size =
            amirstan::common::getAlignedSize(batch_size * nOutputPlane * outputHeight * outputWidth * sizeof_dtype);
                                         ...
                                         cudaStream_t                      stream) PLUGIN_NOEXCEPT
{
    ...
    deform_conv_forward_cuda((float*)inputs[0],
                             (float*)inputs[2],
                             nullptr,
                             (float*)inputs[1],
                             (float*)outputs[0],
...
nvinfer1::DataType DeformableConvPluginDynamic::getOutputDataType(int                       index,
...
const char* DeformableConvPluginDynamic::getPluginVersion() const PLUGIN_NOEXCEPT
{
    return DCN_VERSION;
}
...
size_t DeformableConvPluginDynamic::getSerializationSize() const PLUGIN_NOEXCEPT
{
    return sizeof(mStride) + sizeof(mPadding) + sizeof(mDilation) + sizeof(mDeformableGroup) + sizeof(mGroup);
}

void DeformableConvPluginDynamic::serialize(void* buffer) const PLUGIN_NOEXCEPT
{
...
void DeformableConvPluginDynamic::attachToContext(cudnnContext*            cudnnContext,
...
{
    mPluginAttributes = std::vector<PluginField>({PluginField("stride"),
                                                  PluginField("padding"),
...
const char* DeformableConvPluginDynamicCreator::getPluginName() const PLUGIN_NOEXCEPT
{
    return DCN_NAME;
}

const char* DeformableConvPluginDynamicCreator::getPluginVersion() const PLUGIN_NOEXCEPT
{
    return DCN_VERSION;
}
...
IPluginV2* DeformableConvPluginDynamicCreator::createPlugin(const char*                  name,
