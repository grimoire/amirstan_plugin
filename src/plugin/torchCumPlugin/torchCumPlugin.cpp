...
TorchCumPluginDynamic::TorchCumPluginDynamic(const std::string& name, int dim, int cumType):
...
nvinfer1::IPluginV2DynamicExt* TorchCumPluginDynamic::clone() const PLUGIN_NOEXCEPT
{
    TorchCumPluginDynamic* plugin = new TorchCumPluginDynamic(mLayerName, mDim, mCumType);
    plugin->setPluginNamespace(getPluginNamespace());

    return plugin;
}

nvinfer1::DimsExprs TorchCumPluginDynamic::getOutputDimensions(int                        outputIndex,
...
bool TorchCumPluginDynamic::supportsFormatCombination(int                               pos,
    ...
    switch (pos) {
        case 0:
            return (in[0].type == nvinfer1::DataType::kFLOAT && in[0].format == nvinfer1::TensorFormat::kLINEAR)
                   or (in[0].type == nvinfer1::DataType::kINT32 && in[0].format == nvinfer1::TensorFormat::kLINEAR);
        case 1:
...
void TorchCumPluginDynamic::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* inputs,
                                            int                                      nbInputs,
...
size_t TorchCumPluginDynamic::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs,
                                               int                               nbInputs,
                                   ...
                                   cudaStream_t                      stream) PLUGIN_NOEXCEPT
{
    ...
    switch (data_type) {
        case nvinfer1::DataType::kFLOAT:
            torch_cum<float>(
                (float*)outputs[0], (float*)inputs[0], &(input_dims.d[0]), input_dims.nbDims, mDim, mCumType, stream);
            break;
        case nvinfer1::DataType::kINT32:
            torch_cum<int>(
                (int*)outputs[0], (int*)inputs[0], &(input_dims.d[0]), input_dims.nbDims, mDim, mCumType, stream);
            break;
        default:
            return 1;
            break;
...
nvinfer1::DataType TorchCumPluginDynamic::getOutputDataType(int                       index,
...
int TorchCumPluginDynamic::getNbOutputs() const PLUGIN_NOEXCEPT
{
    return 1;
}
...
const char* TorchCumPluginDynamicCreator::getPluginName() const PLUGIN_NOEXCEPT
{
    return PLUGIN_NAME;
}

const char* TorchCumPluginDynamicCreator::getPluginVersion() const PLUGIN_NOEXCEPT
{
    return PLUGIN_VERSION;
}

IPluginV2* TorchCumPluginDynamicCreator::createPlugin(const char* name, const PluginFieldCollection* fc) PLUGIN_NOEXCEPT
{
    int dim     = 0;
    int cumType = 0;
