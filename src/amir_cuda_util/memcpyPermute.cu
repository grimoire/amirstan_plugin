...
__global__ void copy_permute_kernel(value_type*       dst,
...
template<class value_type>
void memcpyPermute(
    value_type* dst, const value_type* src, int* src_size, int* permute, int src_dim, cudaStream_t stream)
{
    size_t     copy_size = 1;
    TensorSize ts_permute;
    ...
    copy_permute_kernel<value_type><<<GET_BLOCKS(copy_size), CUDA_NUM_THREADS, 0, stream>>>(
        dst, src, copy_size, ts_src_stride, ts_dst_stride, ts_permute, src_dim);
}

template void
memcpyPermute<float>(float* dst, const float* src, int* src_size, int* permute, int src_dim, cudaStream_t stream);
