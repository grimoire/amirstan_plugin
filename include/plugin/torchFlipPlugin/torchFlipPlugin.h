...
{
public:
    TorchFlipPluginDynamic(
        const std::string &name, const nvinfer1::Dims &dims);

    TorchFlipPluginDynamic(const std::string name, const void *data, size_t length);

    // It doesn't make sense to make TorchFlipPluginDynamic without arguments, so we
